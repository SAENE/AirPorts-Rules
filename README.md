...
- **`Clash-Auto-GeoSite.ini`**
    - **特性**：完全依赖 `GeoSite` 和 `GeoIP-Lite` 数据库进行分流。
    - **优势**：配置文件极小，无需维护冗长的规则列表，加载速度极快。
- **`Clash-Auto-Lite.ini`**
    - **特性**：完全依赖 `GeoSite-Lite` 、 `GeoIP-Lite` 和 `Rule-SET` 进行分流，推荐搭配`all_base-Lite.tpl`使用。
    - **优势**：整体文件极小，无需维护冗长的规则列表，加载速度极快。
- **`Clash-Auto-Rule.ini`**
    - **特性**：完全依赖 `Rule-SET` 进行分流，推荐搭配`all_base-Lite.tpl`使用。
    - **优势**：整体文件小，无需维护冗长的规则列表，加载速度极快。
...
- **`all_base-Lite.tpl`**
    - 适用于 SubConverter-Extended 的Lite模板，定义了标准的代理组结构和全局逻辑。
